...
//    assertEquals("N;", PHP.serialize(null, out));
...
//    for (int a = 1; a < 3; a++)
//      assertEquals("i:" + a + ";", PHP.serialize(a, out));
//    for (int i = 1; i < 101; i++) {
//      int ii = PHP.unserialize(Integer.class, "i:" + i + ";");
//      assertEquals(i, ii);
//    }
//    long l = 922372036854775807L;
//    assertEquals("i:" + l + ";", PHP.serialize(l, out));
//    assertEquals(l, (long) PHP.unserialize(Long.class, PHP.serialize(l, out)));
//    byte b = 21;
//    assertEquals("i:" + b + ";", PHP.serialize(b, out));
//    assertEquals(b, (byte) PHP.unserialize(Byte.class, PHP.serialize(b, out)));
//    short s = 32767;
//    assertEquals("i:" + s + ";", PHP.serialize(s, out));
//    assertEquals(s, (short) PHP.unserialize(Short.class, PHP.serialize(s, out)));
//    char c = 'a';
//    assertEquals("s:1:\"a\";", PHP.serialize(c, out));
//    char cc = PHP.unserialize(Character.class, PHP.serialize(c, out));
//    assertEquals(c, cc);
//    String tt = "Test string";
//    assertEquals("s:11:\"Test string\";", PHP.serialize(tt, out));
//    assertEquals(tt, PHP.unserialize(String.class, PHP.serialize(tt, out)));
//    // Enum'ы
//    TestEnum te = TestEnum.Value1;
//    assertEquals("s:6:\"Value1\";", PHP.serialize(te, out));
//    assertEquals(te, PHP.unserialize(TestEnum.class, PHP.serialize(te, out)));
//  }
//
//  @Test
...
//    float f = 1.3232f;
//    assertEquals("d:1.3232;", PHP.serialize(f, out));
//    assertEquals(f, (float) PHP.unserialize(Float.class, PHP.serialize(f, out)));
//    double d = 2.34;
//    assertEquals("d:2.34;", PHP.serialize(d, out));
//    assertEquals(d, (double) PHP.unserialize(Double.class, PHP.serialize(d, out)));
//
//    // Булевский тип переменных
//    assertEquals("b:1;", PHP.serialize(true, out));
//    assertEquals(true, PHP.unserialize(Boolean.class, "b:1;"));
//    assertEquals("b:0;", PHP.serialize(false, out));
//    assertEquals(false, PHP.unserialize(Boolean.class, "b:0;"));
//
//    // Одномерные массивы
//    int[] ar = { 2, 3, 9 };
//    assertEquals("a:3:{i:0;i:2;i:1;i:3;i:2;i:9;}", PHP.serialize(ar, out));
//    int[] arr = PHP.unserialize(int[].class, PHP.serialize(ar, out));
//    assertArrayEquals(ar, arr);
//
//    String[] sa = { "s1", "s2" };
//    assertEquals("a:2:{i:0;s:2:\"s1\";i:1;s:2:\"s2\";}", PHP.serialize(sa, out));
//    String[] saa = PHP.unserialize(String[].class, PHP.serialize(sa, out));
//    assertArrayEquals(sa, saa);
//
//    char[] ca = { 'a', 'b' };
//    assertEquals("a:2:{i:0;s:1:\"a\";i:1;s:1:\"b\";}", PHP.serialize(ca, out));
//    char[] caa = PHP.unserialize(char[].class, PHP.serialize(ca, out));
//    assertArrayEquals(ca, caa);
//
//    // Многомерные массивы
//    int[][] mda = { { 1, 2 }, { 3, 4 } };
//    assertEquals("a:2:{i:0;a:2:{i:0;i:1;i:1;i:2;}i:1;a:2:{i:0;i:3;i:1;i:4;}}",
//        PHP.serialize(mda, out));
//    int[][] mdaa = PHP.unserialize(mda.getClass(), PHP.serialize(mda, out));
...
//        PHP.serialize(hm, out));
//    HSS hma = PHP.unserialize(HSS.class, PHP.serialize(hm, out));
...
//        .serialize(m, out));
//    MyClass ma = PHP.unserialize(MyClass.class, PHP.serialize(m, out));
...
//        PHP.serialize(bc, out));
//    BigClass bca = PHP.unserialize(BigClass.class, PHP.serialize(bc, out));
